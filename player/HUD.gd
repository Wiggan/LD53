...
func _process(_delta):
	if net_scale != Globals.net_scale:
		...
		$VBoxContainer/NetSize.text = "[update]Net Size: " + "%.2f" % net_scale + "[/update]"
	...
	if net_impulse != Globals.net_impulse:
		...
		$VBoxContainer/NetImpulse.text = "[update]Power: " + "%.2f" % net_impulse + "[/update]"
		$LootBoxNotification/AnimationPlayer.play("show")
	if net_cooldown != Globals.net_cooldown:
		...
		$VBoxContainer/NetCooldown.text = "[update]Cooldown: " + "%.2f" % net_cooldown + "[/update]"
		$LootBoxNotification/AnimationPlayer.play("show")
